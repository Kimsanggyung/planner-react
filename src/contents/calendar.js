...
function Calendar({stateData, setStateData, dateDate, setDateData ,loggedUser, date, setDate, targetID, setTargetID, token}){
  ...
  if(stateData.loginState && stateData.selectedCalendar === "daily"){ // 로그인상태이면서 dailyState가 참이면 selectedCalendar에 daily컴포넌트를 할당
    selectedCalendar =  <Daily 
                          ...
                          setStateData={setStateData} date={date} setDate={setDate} targetID={targetID} setTargetID={setTargetID}
                        />
    ...
    selectedCalendar =  <Weekly 
                          ...
                          loggedUser={loggedUser} odate={date} setDate={setDate} setTargetID={setTargetID} token={token}
                        />
